...
___Host-adapted virus in FAM and UNFAM___
Female only
BALBF-adapted virus (Host) vs BL6F adapted virus, in both hosts.
```{r}
# Fit the CoxPH model for the current host level
...
Black mice mortality with either virus (fam and unfam) vs balb mortality
```{r}
# Fit the CoxPH model for the current host level
survdatatradeoff <- subset(survdata, Sex=="F" & Treatment != "STOCK" & Treatment !="1XP")
...
Comparison between host-adapted viruses in fam and unfam.
BALBF-adapted virus (Host) vs BL6F adapted virus, in both hosts.
```{r}
